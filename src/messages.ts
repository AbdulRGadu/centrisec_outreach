...
export async function handleMessagePatch(id: string, body: Record<string, unknown>, env: Env): Promise<Response> {
  ...
  const newBody = typeof body.body === 'string' ? normalizeMultiline(body.body, 5000) : null;
...
export async function handleMessageApprove(id: string, env: Env): Promise<Response> {
  const message = await getMessage(env, id);
  if (message.status !== 'draft') throw new HttpError(409, `Cannot approve a message in status '${message.status}'`);
  const lead = await assertSendable(env, message);

  const claim = await env.DB
    .prepare(`UPDATE messages SET status = 'approved', updated_at = datetime('now') WHERE id = ?1 AND status = 'draft'`)
    .bind(id)
    .run();
...
export async function handleMessageReject(id: string, body: Record<string, unknown>, env: Env): Promise<Response> {
  const message = await getMessage(env, id);
  if (message.status !== 'draft') throw new HttpError(409, `Cannot reject a message in status '${message.status}'`);
  const reason = normalizeText(body.reason, 300) || null;
  await env.DB
    .prepare(`UPDATE messages SET status = 'rejected', error = ?1, updated_at = datetime('now') WHERE id = ?2 AND status = 'draft'`)
    .bind(reason, id)
    .run();
  if (message.lead_id) {
    await env.DB
...
export async function handleSendNow(id: string, env: Env): Promise<Response> {
  const message = await getMessage(env, id);
  if (!['draft', 'approved', 'queued'].includes(message.status)) {
